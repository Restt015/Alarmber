...
// Datos mock de alertas (debería venir de una fuente compartida, pero por simplicidad lo duplicamos)
...
const InfoSection = ({ title, content }: { title: string; content: string }) => {
  return (
    <View className="mb-5">
      ...
      <Text className="text-base text-[#AAAAAA] leading-6">{content}</Text>
    </View>
  );
};
  ...
  return (
    <ScrollView className="flex-1 bg-[#121212]" showsVerticalScrollIndicator={false}>
      <View className="relative w-full h-[300px]">
        <Image source={{ uri: alert.photo }} className="w-full h-full bg-[#1E1E1E]" />
        <View className="absolute top-4 right-4 bg-[#D32F2F] flex-row items-center px-3 py-1.5 rounded-full">
          <Ionicons name="time-outline" size={16} color="#FFFFFF" />
          <Text className="text-white text-xs font-bold ml-1">{alert.daysMissing} días desaparecido/a</Text>
        </View>
      </View>

      <View className="p-5">
        <Text className="text-[28px] font-bold text-white mb-1.5">{alert.name}</Text>
        <Text className="text-lg text-[#AAAAAA] mb-4">Edad: {alert.age} años</Text>

        <View className="flex-row items-center bg-[#1E1E1E] p-3 rounded-lg mb-5">
        ...
        <TouchableOpacity 
          className="bg-[#0D47A1] flex-row items-center justify-center p-4.5 rounded-xl mt-2.5 mb-5"
          onPress={handleCall}
        >
        ...
        <View className="flex-row bg-[#1E1E1E] p-4 rounded-xl border-l-4 border-l-[#D32F2F]">
